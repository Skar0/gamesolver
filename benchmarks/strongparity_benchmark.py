...
from solvers.strongparity import strong_parity_solver, strong_parity_solver_non_removed, strong_parity_antichain_based
...
def benchmark_random_antichain_based(n, iterations=3, step=10, plot=False, path=""):
    ...
    if plot:
        plt.grid(True)
        plt.title(u"Random graphs of size 1 à " + str(n))
...
def benchmark_worst_case_antichain_based(n, iterations=3, step=10, plot=False, path=""):
    ...
    for i in range(1, n + 1, step):
        ...
        for j in range(iterations):
            with chrono:
                strong_parity_antichain_based(g,1)  # solver call
            temp.append(chrono.interval)  # add time recording
