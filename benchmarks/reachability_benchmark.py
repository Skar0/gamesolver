...
def benchmark(n, generator, t, p, iterations=3, step=10, plot=False, regression=False, order=1, path=""):
    ...
    info = "Reachability player " + str(p) + " target " + str(t)  # info about the current benchmark
    ...
    for i in range(1, n + 1, step):
        ...
        # at the end, print total time
    print "-" * 108 + "\n" + "total".center(40) + "|" + "#".center(12) + "|" + \
          str(total_time).center(40) + "\n" + "-" * 108 + "\n"

    if plot:
        plt.grid(True)
        plt.title(u"Generator : " + str(generator.__name__).replace("_", " "))
        plt.xlabel(u'nombre de nœuds')
        plt.ylabel(u'temps (s)')
        if regression:
...
# benchmark_complete_graph(1000,iterations=1,step=10,plot=True,path="../resu/")
# benchmark_worst_case(1000,iterations=1,step=10,plot=True,path="../resu/")
